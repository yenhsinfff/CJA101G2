...
public class DiscountCodeService {
	...
	//修改折價券
	@Transactional
	public void updateDiscountCode(DiscountCodeDTO_update dto) {
		Optional<DiscountCodeVO> optional = repository.findById(dto.getDiscountCodeId());
		
		DiscountCodeVO vo = optional.orElseThrow(() -> 
	    new RuntimeException("找不到對應的折價券 ID：" + dto.getDiscountCodeId()));
		
	
		// DTO 轉 VO，存入Entity
		vo = optional.get();
		vo.setDiscountCodeId(dto.getDiscountCodeId());
		vo.setDiscountCode(dto.getDiscountCode());
		vo.setDiscountType(dto.getDiscountType());
		vo.setDiscountValue(dto.getDiscountValue());
		vo.setDiscountExplain(dto.getDiscountExplain());
...
//	}
	...
	public String getNextDiscountCodeId(String prefix) {
		...
		// 過濾出指定 prefix 的折價券
		Optional<Integer> maxNumber = allCodes.stream().map(DiscountCodeVO::getDiscountCodeId)
				.filter(id -> id != null && id.startsWith(prefix)).map(id -> {
					try {
						return Integer.parseInt(id.substring(1));
					} catch (Exception e) {
						return null;
					}
				}).filter(num -> num != null).max(Comparator.naturalOrder());
