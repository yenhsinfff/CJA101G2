...
public class ProdColorListController {
    ...
    @GetMapping("/{prodId}")
    public ApiResponse<List<ProdColorListDTO>> getColorsByProdId(@PathVariable Integer prodId) {
        List<ProdColorListDTO> colorList = service.findByProdIdDTO(prodId);
    ...
    public ApiResponse<ProdColorListDTO> getOne(@PathVariable Integer prodId, @PathVariable Integer colorId) {
        ProdColorListDTO dto = service.findOneDTO(prodId, colorId);
        if (dto != null) {
    ...
    @PostMapping("")
    public ApiResponse<ProdColorListDTO> save(@RequestBody ProdColorListDTO dto) {
